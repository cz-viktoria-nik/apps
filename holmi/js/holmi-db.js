...
// Tipus lista frissitese
function refreshTypeList() {
    // editTypesView - tipus szerkesztesi oldal
    var list = document.getElementById("typelist");
    list.innerHTML = "";
    
    // actionView - uj kolcsonzes felviteli oldal
    ...
    // listByTypeView - tipus szerinti listazas
    ...
    // editActionView - kolcsonzes szerkeszto oldal
...
// kolcsonzesi bejegyzes szerkesztese
function viewItem( item_id ) {
    showEditActionView();
    
    // valasztott elem adatainak betoltese:
...
function addBorrowLendItem() {
    ...
    if( checkInputs( 'add' )) {
        ...
        return db.transaction("rw", db.items, function () {
            ...
            //console.log(borrowLendItem.notify_time);
            resetInputFields();
            refreshItemCounters();
        }).catch(function (e) {
            console.log(e, "error");
...
function refreshItemCounters() {   
    // Megjegyzes: a kolcsonadott / kert / visszajuttatok targyak darabszamait 
    // kellene itt frissiteni, de a .count()-ot nem sikerult mukodesre birni, 
    // globalis valtozoval szinten nem ment... ezert ez a ronda "megoldas"!
...
function createNotification(item) {
    ...
    var img = 'img/icon-30.png'; // nem jelenik meg :(
    ...
    return db.transaction("rw", db.items, function () {
        db.items.each(function (item) {
            if(item.notify_time != "" && item.notified != 1 ) {
                //console.log('NOTIFY: ' + item.notify_date + " " + item.notify_time + " --> " + item.notified);
                ...
                if( ( hours == hourCheck ) && ( minutes == minuteCheck ) && (day == dayCheck) && (month == monthCheck ) && (year == yearCheck)) {
                    // ha a datum stimmel -> uzenet, es notified = true (1)
